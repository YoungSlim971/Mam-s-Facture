...
export async function generateInvoicePDF(
  ...
  doc.text(
    currency.format(totals.totalHT),
    boxX + boxWidth - 5 - doc.widthOfString(currency.format(totals.totalHT)),
    doc.y
  );
  doc.text(`\nTVA (${parsed.tvaRate ?? 20} %) :`, boxX + 5, doc.y, { continued: true });
  doc.text(
    currency.format(totals.totalTVA),
    boxX + boxWidth - 5 - doc.widthOfString(currency.format(totals.totalTVA)),
    doc.y
  );
  ...
  doc.text(
    ...
    boxX + boxWidth - 5 - doc.widthOfString(currency.format(totals.totalTTC)),
    doc.y
  );
